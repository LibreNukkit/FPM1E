...
BlockEntityNameable {
    ...
    protected void initBlockEntity() {
        ...
        for (Tag compoundTag : listTag.getAll()) {
            Item item = NBTIO.getItemHelper((CompoundTag) compoundTag);
            if (item.getId() == 0 || item.getCount() <= 0) continue;
            this.inventory.slots.put(((CompoundTag) compoundTag).getByte("Slot"), item);
        }
