...
extends InventoryAction {
    ...
    public boolean isValid(Player player) {
        ...
        if (this.inventory.getHolder() instanceof BlockEntityContainer && (((BlockEntity) this.inventory.getHolder()).distanceSquared(player) > 4096.0 || !((BlockEntity) this.inventory.getHolder()).getLevel().equals(player.getLevel()))) {
        ...
        if (this.inventory.getHolder() != player && this.inventory.getHolder() instanceof Entity && (((Entity) this.inventory.getHolder()).distanceSquared(player) > 4096.0 || !((Entity) this.inventory.getHolder()).getLevel().equals(player.getLevel()))) {
            player.getServer().getLogger().debug(player.getName() + ": got SlotChangeAction but player is too far away from the holder of " + this.inventory);
            return false;
        }
    ...
    public void onExecuteSuccess(Player player) {
        int n;
        HashSet<Player> hashSet = new HashSet<>(this.inventory.getViewers());
