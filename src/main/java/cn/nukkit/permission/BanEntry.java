...
public class BanEntry {
    ...
    public static BanEntry fromString(String string) {
        Map map = new Gson().fromJson(string, new TreeMapTypeToken(null).getType());
        ...
        try {
            ...
            banEntry.setExpirationDate(!map.get("expireDate").equals("Forever") ? new SimpleDateFormat("yyyy-MM-dd HH:mm:ss Z").parse((String)map.get("expireDate")) : null);
        }
        catch (ParseException parseException) {
            Server.getInstance().getLogger().logException(parseException);
        }
