...
RakNetSessionListener {
    ...
    public void onEncapsulated(EncapsulatedPacket encapsulatedPacket) {
        ...
        if (s2 == 254) {
            ...
            try {
                ...
                h.error("[{}] Unable to process batch packet", this.b == null ? this.c.getAddress() : this.b.getName(), exception);
            }
        }
    }
    ...
    public void sendPacket(DataPacket dataPacket) {
        ...
        if (dataPacket.protocol != this.b.protocol) {
            h.warn("Wrong protocol used for {}! expected {} got{}", dataPacket.getClass().getSimpleName(), this.b.protocol, dataPacket.protocol);
        }
        if (dataPacket.pid() != -1) {
    ...
    private void a() {
        ...
        try {
            ...
            h.error("[{}] Failed to tick RakNetPlayerSession", this.c.getAddress(), exception);
        }
    }
    ...
    private void a(Collection<DataPacket> collection) {
        ...
        try {
            ...
            h.error("Unable to compress batched packets", exception);
        }
    }
