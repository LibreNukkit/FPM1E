...
extends DataPacket {
    ...
    private final List<AbilityLayer> h = new ObjectArrayList<>();
    ...
    private static int a(Set<PlayerAbility> set) {
        ...
        for (PlayerAbility playerAbility : set) {
            n |= i.getOrDefault(playerAbility, 0).intValue();
        }
        return n;
    }
    ...
    public String toString() {
        return "UpdateAbilitiesPacket(entityId=" + this.getEntityId() + ", playerPermission=" + this.getPlayerPermission() + ", commandPermission=" + this.getCommandPermission() + ", abilityLayers=" + this.getAbilityLayers() + ")";
    }
