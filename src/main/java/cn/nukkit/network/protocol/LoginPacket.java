...
extends DataPacket {
    ...
    private void c() {
        ...
        if ((map = e.fromJson(string = new String(this.get(n), StandardCharsets.UTF_8), new MapTypeToken(null).getType())).isEmpty() || !map.containsKey("chain") || ((List<?>)map.get("chain")).isEmpty()) {
            return;
        }
        long l = System.currentTimeMillis();
        for (String string2 : (List<String>)map.get("chain")) {
