...
public class Network {
    ...
    private final Set<SourceInterface> c = new HashSet<>();
    private final Set<AdvancedSourceInterface> d = new HashSet<>();
    ...
    public void processInterfaces() {
        for (SourceInterface sourceInterface : this.c) {
            try {
                ...
                b.fatal(this.i.getLanguage().translateString("nukkit.server.networkError", new String[]{sourceInterface.getClass().getName(), Utils.getExceptionMessage(exception)}), exception);
            }
        }
    }
    ...
    public void processBatch(BatchPacket batchPacket, Player player) {
        ...
        try {
            ...
            b.error("Error whilst decoding batch packet from " + player.getName(), exception);
    ...
    public void processBatch(byte[] byArray, Collection<DataPacket> collection, CompressionProvider compressionProvider, int n, int n2) throws Exception {
        ...
        while (!binaryStream.feof()) {
            ...
            if (dataPacket == null) {
                ...
                b.debug("Received unknown packet with ID: {}", Integer.toHexString(n4));
                continue;
            }
