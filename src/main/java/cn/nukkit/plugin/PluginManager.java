...
public class PluginManager {
    ...
    public Map<String, Plugin> loadPlugins(File file2, List<String> list, boolean bl) {
        if (file2.isDirectory()) {
            ...
            LinkedHashMap<String, Plugin> linkedHashMap2 = new LinkedHashMap<>();
            ...
            Map<String, PluginLoader> map = new LinkedHashMap<>();
            if (list != null) {
            ...
            for (PluginLoader pluginLoader : map.values()) {
                for (File file3 : Objects.requireNonNull(file2.listFiles((file, string) -> {
                    for (Pattern pattern : pluginLoader.getPluginFilters()) {
                    ...
                    try {
                        ...
                        var object = pluginDescription.getName();
            ...
            while (!linkedHashMap.isEmpty()) {
                ...
                for (String string3 : new ArrayList<>(linkedHashMap.keySet())) {
                    File file4 = linkedHashMap.get(string3);
                    if (linkedHashMap3.containsKey(string3)) {
                        for (String string4 : new ArrayList<>(linkedHashMap3.get(string3))) {
                            if (linkedHashMap2.containsKey(string4) || this.getPlugin(string4) != null) {
                                linkedHashMap3.get(string3).remove(string4);
                                continue;
                            }
                        ...
                        if (linkedHashMap3.get(string3).isEmpty()) {
                            linkedHashMap3.remove(string3);
                        }
                    }
                    if (linkedHashMap4.containsKey(string3)) {
                        for (String string5 : new ArrayList<>(linkedHashMap4.get(string3))) {
                            if (!linkedHashMap2.containsKey(string5) && this.getPlugin(string5) == null) continue;
                            linkedHashMap4.get(string3).remove(string5);
                        }
                        if (linkedHashMap4.get(string3).isEmpty()) {
                ...
                for (String string6 : new ArrayList<>(linkedHashMap.keySet())) {
                    File file5 = linkedHashMap.get(string6);
    ...
    public Set<Permissible> getPermissionSubscriptions(String string) {
        if (this.permSubs.containsKey(string)) {
            return new HashSet<Permissible>(this.permSubs.get(string));
        }
        return new HashSet<>();
    }
    ...
    protected List<PluginCommand> parseYamlCommands(Plugin plugin) {
        ...
        for (Map.Entry<String, Object> entry : plugin.getDescription().getCommands().entrySet()) {
            ...
            PluginCommand<Plugin> pluginCommand = new PluginCommand<>(string, plugin);
            ...
            if (((Map<?, ?>)object).containsKey("aliases") && (v = ((Map<?, ?>)object).get("aliases")) instanceof List) {
                ...
                for (String string2 : (List<String>)v) {
                    if (string2.contains(":")) {
    ...
    public void disablePlugins() {
        ListIterator<Plugin> listIterator = new ArrayList<>(this.plugins.values()).listIterator(this.plugins.size());
    ...
    public void registerEvents(Listener listener, Plugin plugin) {
        ...
        for (Object object : hashSet) {
            ...
            Class<? extends Event> clazz2 = clazz.asSubclass(Event.class);
