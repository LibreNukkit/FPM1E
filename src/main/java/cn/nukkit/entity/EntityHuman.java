...
extends EntityHumanType {
        ...
        if (!(this instanceof Player)) {
            ...
            if (this.namedTag.contains("Skin") && this.namedTag.get("Skin") instanceof CompoundTag) {
                ...
                if (compoundTag.contains("Data")) {
                    var object = compoundTag.getByteArray("Data");
                ...
                if (compoundTag.contains("CapeData")) {
                    var object = compoundTag.getByteArray("CapeData");
                ...
                if (compoundTag.contains("AnimatedImageData")) {
                    for (CompoundTag iterator : compoundTag.getList("AnimatedImageData", CompoundTag.class).getAll()) {
                        skin.getAnimations().add(new SkinAnimation(new SerializedImage(iterator.getInt("ImageWidth"), ((CompoundTag) iterator).getInt("ImageHeight"), ((CompoundTag) iterator).getByteArray("Image")), ((CompoundTag) iterator).getInt("Type"), ((CompoundTag) iterator).getFloat("Frames"), ((CompoundTag) iterator).getInt("AnimationExpression")));
                    }
                }
                ...
                if (compoundTag.contains("PersonaPieces")) {
                    var object = compoundTag.getList("PersonaPieces", CompoundTag.class);
                    for (CompoundTag compoundTag2 : ((ListTag<CompoundTag>)object).getAll()) {
                ...
                if (compoundTag.contains("PieceTintColors")) {
                    var object = compoundTag.getList("PieceTintColors", CompoundTag.class);
                    for (CompoundTag compoundTag2 : ((ListTag<CompoundTag>)object).getAll()) {
    ...
    public void saveNBT() {
        ...
        if (this.skin != null) {
            ...
            var listTag2 = this.getSkin().getTintColors();
            if (!(listTag2).isEmpty()) {
                var listTag = new ListTag<>("PieceTintColors");
                for (PersonaPieceTint personaPieceTint : listTag2) {
                    var listTag3 = new ListTag<>("Colors");
                    listTag3.setAll(personaPieceTint.colors.stream().map(string -> new StringTag("", string)).collect(Collectors.toList()));
