...
extends EntityProjectile {
    ...
    protected void initEntity() {
        ...
        if (this.namedTag.contains("CollisionPos")) {
            var listTag = this.namedTag.getList("CollisionPos", DoubleTag.class);
            this.n = new Vector3(listTag.get(0).data, listTag.get(1).data, ((DoubleTag)listTag.get(2)).data);
        } else {
            this.n = r.clone();
        }
        if (this.namedTag.contains("StuckToBlockPos")) {
            var listTag = this.namedTag.getList("StuckToBlockPos", IntTag.class);
            this.l = new BlockVector3(((IntTag)listTag.get(0)).data, ((IntTag)listTag.get(1)).data, ((IntTag)listTag.get(2)).data);
        } else {
            this.l = m.clone();
        }
        this.o = this.namedTag.contains("favoredSlot") ? this.namedTag.getInt("favoredSlot") : -1;
        this.p = !this.namedTag.contains("player") || this.namedTag.getBoolean("player");
    }
