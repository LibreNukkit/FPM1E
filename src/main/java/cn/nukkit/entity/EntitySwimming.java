...
extends BaseEntity {
    ...
    public Vector3 updateMove(int n) {
        if (this.isMovement() && !this.isImmobile()) {
            ...
            d3 = this.motionX;
            d2 = this.motionZ;
            this.motionY = this.isInsideOfWater() && (this.motionX > 0.0 || this.motionZ > 0.0) ? Utils.rand(-0.12, 0.12) : (!this.isOnGround() && !this.isInsideOfWater() ? (this.motionY -= this.getGravity()) : 0.0);
            if (this.stayTime > 0) {
                this.stayTime -= n;
