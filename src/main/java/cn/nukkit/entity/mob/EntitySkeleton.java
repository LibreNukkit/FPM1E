...
implements EntitySmite {
    ...
    public EntitySkeleton(FullChunk fullChunk, CompoundTag compoundTag) {
    ...
    public void attackEntity(Entity entity) {
        if (this.attackDelay > 23 && Utils.rand(1, 32) < 4 && this.distanceSquared(entity) <= 55.0) {
            ...
            if (this.getLevel().getBlockIdAt(this.chunk, location.getFloorX(), location.getFloorY(), location.getFloorZ()) == 0) {
                Entity entity2 = Entity.createEntity("Arrow", location, this);
                if (!(entity2 instanceof EntityArrow)) {
                    return;
                }
                EntityArrow entityArrow = (EntityArrow)entity2;
                EntitySkeleton.setProjectileMotion(entityArrow, d4, d5, d6, d2);
