...
implements EntityDamageable {
    ...
    protected boolean blockedByShield(EntityDamageEvent entityDamageEvent) {
        ...
        entity = entityDamageEvent instanceof EntityDamageByChildEntityEvent ? ((EntityDamageByChildEntityEvent)entityDamageEvent).getChild() : (entity = entityDamageEvent instanceof EntityDamageByEntityEvent ? ((EntityDamageByEntityEvent)entityDamageEvent).getDamager() : null);
    ...
    public void kill() {
        ...
        var damager = ((EntityDamageByEntityEvent)entityDeathEvent.getEntity().getLastDamageCause()).getDamager();
        if ((n == 38 || n == 36 || n == 35 || n == 40) && entityDeathEvent.getEntity().getLastDamageCause() instanceof EntityDamageByEntityEvent && damager instanceof Player) {
        ...
        if (this.level.getGameRules().getBoolean(GameRule.DO_MOB_LOOT) && this.lastDamageCause != null && EntityDamageEvent.DamageCause.VOID != this.lastDamageCause.getCause()) {
            var dgs = (BaseEntity)entityDeathEvent.getEntity();
            if (entityDeathEvent.getEntity() instanceof BaseEntity && dgs.getLastDamageCause() instanceof EntityDamageByEntityEvent) {
                Entity entity = ((EntityDamageByEntityEvent)dgs.getLastDamageCause()).getDamager();
                if (entity instanceof Player) {
                    this.getLevel().dropExpOrb(this, dgs.getKillExperience());
                ...
                } else if (dgs instanceof EntityCreeper && (entity instanceof EntitySkeleton || entity instanceof EntityStray) && this.lastDamageCause.getCause() == EntityDamageEvent.DamageCause.PROJECTILE) {
                    this.getLevel().dropItem(this, Item.get(Utils.rand(500, 511), 0, 1));
                }
            }
            if (this.dropsOnNaturalDeath()) {
                int var5_10 = 0;
                var ddj = entityDeathEvent.getDrops();
                int n2 = ddj.length;
                boolean bl = false;
                while (var5_10 < n2) {
                    Item item = ddj[var5_10];
                    this.getLevel().dropItem(this, item);
                    ++var5_10;
                }
            }
    ...
    public boolean entityBaseTick(int n) {
        ...
        if (this instanceof Player && !this.closed) {
            ...
            n2 = !bl ? 1 : 0;
            var entityArray = player.getInventory();
        ...
        if (this.isAlive()) {
            if (this.isInsideOfSolid()) {
                bl2 = true;
                this.attack(new EntityDamageEvent(this, EntityDamageEvent.DamageCause.SUFFOCATION, 1.0f));
            }
            ...
            if (bl && !this.hasEffect(13)) {
                if (this instanceof EntitySwimming || this.isDrowned || this instanceof Player && (((Player)this).isCreative() || ((Player)this).isSpectator())) {
                    ...
                    if (n2 <= -20) {
                        ...
                        if (!(this instanceof Player) || this.level.getGameRules().getBoolean(GameRule.DROWNING_DAMAGE)) {
                            this.attack(new EntityDamageEvent(this, EntityDamageEvent.DamageCause.DROWNING, 2.0f));
                        }
                    }
                    this.setAirTicks(n2);
                }
                ...
                if (n2 <= -20) {
                    n2 = 0;
                    this.attack(new EntityDamageEvent(this, EntityDamageEvent.DamageCause.SUFFOCATION, 2.0f));
                }
                this.setAirTicks(n2);
            } else {
                n2 = this.getAirTicks();
    ...
    public Block[] getLineOfSight(int n, int n2, Integer[] integerArray) {
        ...
        while (blockIterator.hasNext()) {
            ...
            int n3 = block.getId();
            if (!(integerArray == null ? n3 != 0 : Arrays.binarySearch(integerArray, n3) < 0)) continue;
            break;
        }
    ...
    public Block getTargetBlock(int n, Integer[] integerArray) {
        block4: {
            try {
                ...
                if (integerArray != null && integerArray.length != 0) {
                    if (Arrays.binarySearch(integerArray, block.getId()) < 0) {
                        return block;
                    }
                    break block4;
                }
    ...
    private void a() {
        if (this instanceof EntityTameable) {
            ...
            ((EntityTameable) this).getOwner().dataPacket((DataPacket)cloneable);
        }
    }
