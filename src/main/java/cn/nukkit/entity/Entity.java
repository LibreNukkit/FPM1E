...
implements Metadatable {
    ...
    private static final Map<String, Class<? extends Entity>> g = new HashMap<>();
    ...
    protected final void init(FullChunk fullChunk, CompoundTag compoundTag) {
        ...
        float f3 = listTag2.get(0).data;
        if (!(f3 >= 0.0f) || !(f3 <= 360.0f)) {
            f3 = 0.0f;
        }
        if (!((f2 = listTag2.get(1).data) >= 0.0f) || !(f2 <= 360.0f)) {
            f2 = 0.0f;
        }
        this.setPositionAndRotation(this.temporalVector.setComponents(listTag.get(0).data, listTag.get(1).data, listTag.get(2).data), f3, f2);
        this.setMotion(this.temporalVector.setComponents(listTag3.get(0).data, listTag3.get(1).data, listTag3.get(2).data));
    ...
    public static Entity createEntity(String string, FullChunk fullChunk, CompoundTag compoundTag, Object ... objectArray) {
        ...
        return Entity.wtfOnGround(clazz, fullChunk, compoundTag, objectArray);
    }
    ...
    public static Entity createEntity(int n, FullChunk fullChunk, CompoundTag compoundTag, Object ... objectArray) {
        ...
        if (entityDefinition != null) {
            return Entity.wtfOnGround(entityDefinition.getImplementation(), fullChunk, compoundTag, objectArray);
        }
        return null;
    }

    private static Entity wtfOnGround(Class<? extends Entity> clazz, FullChunk fullChunk, CompoundTag compoundTag, Object ... objectArray) {
        Entity entity = null;
    ...
    public void saveNBT() {
        ...
        if (!this.effects.isEmpty()) {
            ListTag<CompoundTag> listTag = new ListTag<>("ActiveEffects");
            for (Effect effect : this.effects.values()) {
    ...
    public void spawnTo(Player player) {
        ...
        if (!this.hasSpawned.containsKey(player.getLoaderId()) && (bl = player.usedChunks.get(Level.chunkHash(this.chunk.getX(), this.chunk.getZ()))) != null && bl.booleanValue()) {
            ...
            if (this.riding != null) {
                ...
                var dataPacket = new SetEntityLinkPacket();
            ...
            if (this.server.vanillaBossBar && this instanceof EntityBoss) {
                var dataPacket = new BossEventPacket();
    ...
    protected DataPacket createAddEntityPacket() {
        ...
        for (int k = 0; k < addEntityPacket.links.length; ++k) {
            addEntityPacket.links[k] = new EntityLink(this.id, this.passengers.get(k).id, k == 0 ? (byte)1 : 2, false, false);
        }
    ...
    private static boolean wtfOnGround(Player player) {
    ...
    public boolean attack(EntityDamageEvent entityDamageEvent) {
        block18: {
            ...
            var dm = ((EntityDamageByEntityEvent)entityDamageEvent).getDamager();
            if (this instanceof EntityLiving && !(entityDamageEvent instanceof EntityDamageByChildEntityEvent) && dm instanceof Player && Entity.wtfOnGround((Player) dm)) {
            ...
            if (entityDamageEvent.isApplicable(EntityDamageEvent.DamageModifier.CRITICAL)) {
                ...
                pk.action = AnimatePacket.Action.CRITICAL_HIT;
                pk.eid = this.getId();
                this.getLevel().addChunkPacket(this.getChunkX(), this.getChunkZ(), pk);
                this.getLevel().addLevelSoundEvent(this, 43);
            }
            if (entityDamageEvent instanceof EntityDamageByEntityEvent) {
                var enchantmentArray = ((EntityDamageByEntityEvent)entityDamageEvent).getWeaponEnchantments();
    ...
    public void moveFlying(float f2, float f3, float f4) {
        ...
        if (f5 >= 1.0E-4f) {
            ...
            this.motionX += (f2 *= f5) * f7 - (f3 *= f5) * f6;
            this.motionZ += f3 * f7 + f2 * f6;
        }
    }
    ...
    public void respawnToAll() {
        ArrayList<Player> arrayList = new ArrayList<>(this.hasSpawned.values());
    ...
    public static Entity create(Object object, Position position, Object ... objectArray) {
        ...
        return Entity.createEntity(object.toString(), baseFullChunk, compoundTag, objectArray);
    }
