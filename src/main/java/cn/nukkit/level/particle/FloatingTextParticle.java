...
extends Particle {
    ...
    private static final UUID c = UUID.nameUUIDFromBytes(Binary.appendBytes(Skin.GEOMETRY_CUSTOM.getBytes(StandardCharsets.UTF_8), new byte[][]{FloatingTextParticle.a.data}));
    ...
    public DataPacket[] mvEncode(int n) {
        ...
        if (this.entityId == -1L) {
            ...
            var entryArray = new RemoveEntityPacket();
        ...
        if (!this.invisible) {
            var entryArray = new PlayerListPacket.Entry[]{new PlayerListPacket.Entry(this.uuid, this.entityId, this.metadata.getString(4), b)};
