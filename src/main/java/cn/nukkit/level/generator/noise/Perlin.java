...
extends Noise {
    ...
    public Perlin(NukkitRandom nukkitRandom, double d2, double d3) {
        this(nukkitRandom, d2, d3, 1.0);
    }

    public Perlin(NukkitRandom nukkitRandom, double d2, double d3, double d4) {
        int n;
        this.octaves = d2;
        this.persistence = d3;
        this.expansion = d4;
    ...
    public double getNoise3D(double d2, double d3, double d4) {
        ...
        int n5 = n2 & 0xFF;
        int n6 = n3 & 0xFF;
        double d5 = (d2 -= n) * d2 * d2 * (d2 * (d2 * 6.0 - 15.0) + 10.0);
        double d6 = (d3 -= n2) * d3 * d3 * (d3 * (d3 * 6.0 - 15.0) + 10.0);
        double d7 = (d4 -= n3) * d4 * d4 * (d4 * (d4 * 6.0 - 15.0) + 10.0);
        ...
        double d8 = Perlin.grad(this.perm[n9], d2, d3, d4);
        double d9 = Perlin.grad(this.perm[n11], d2 - 1.0, d3, d4);
        double d10 = Perlin.grad(this.perm[n10], d2, d3 - 1.0, d4);
        double d11 = Perlin.grad(this.perm[n12], d2 - 1.0, d3 - 1.0, d4);
        double d12 = Perlin.grad(this.perm[n9 + 1], d2, d3, d4 - 1.0);
        double d13 = Perlin.grad(this.perm[n11 + 1], d2 - 1.0, d3, d4 - 1.0);
        double d14 = Perlin.grad(this.perm[n10 + 1], d2, d3 - 1.0, d4 - 1.0);
        double d15 = Perlin.grad(this.perm[n12 + 1], d2 - 1.0, d3 - 1.0, d4 - 1.0);
