...
public class BlockPalette {
    ...
    private final Map<CompoundTag, Integer> b = new HashMap<>();
    ...
    public int getRuntimeId(int n, int n2) {
        ...
        if (n4 == -1 && (n4 = this.d.get(n << 6)) == -1) {
            n4 = this.d.get(15872);
            e.info("({}) Missing block runtime id mappings for {}:{}", this.f, n, n2);
        }
        return n4;
    }
    ...
    public int getRuntimeId(int n) {
        int n2 = this.d.get(n);
        if (n2 == -1) {
            e.info("({}) Missing block runtime id mappings for {}", this.f, n);
