...
implements LevelProvider {
    ...
    public void unloadChunks() {
        ...
        while (iterator.hasNext()) {
            iterator.next().unload(this.level.getAutoSave(), false);
            iterator.remove();
        }
    }
    ...
    public void doGarbageCollection() {
        ...
        synchronized (this.regions) {
            ...
            Iterator<BaseRegionLoader> iterator = this.regions.values().iterator();
            while (iterator.hasNext()) {
                BaseRegionLoader baseRegionLoader = iterator.next();
    ...
    public boolean unloadChunk(int n, int n2, boolean bl) {
        ...
        synchronized (this.chunks) {
            ...
            if (baseFullChunk != null && baseFullChunk.unload(false, bl)) {
                this.b.set(null);
                this.chunks.remove(l, baseFullChunk);
                return true;
            }
        }
        return false;
    ...
    public BaseFullChunk getLoadedChunk(int n, int n2) {
        ...
        long l = Level.chunkHash(n, n2);
        synchronized (this.chunks) {
            baseFullChunk = this.chunks.get(l);
    ...
    public void setChunk(int n, int n2, FullChunk fullChunk) {
        ...
        long l = Level.chunkHash(n, n2);
        synchronized (this.chunks) {
            if (this.chunks.containsKey(l) && !this.chunks.get(l).equals(fullChunk)) {
