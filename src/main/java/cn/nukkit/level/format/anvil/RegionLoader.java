...
extends BaseRegionLoader {
    ...
    protected boolean isChunkGenerated(int n) {
        Integer[] integerArray = this.locationTable.get(n);
    ...
    public Chunk readChunk(int n, int n2) throws IOException {
        ...
        try {
            Integer[] integerArray = this.locationTable.get(n3);
    ...
    protected void saveChunk(int n, int n2, byte[] byArray) throws IOException {
        block5: {
            ...
            boolean bl = false;
            Integer[] integerArray = this.locationTable.get(n5);
    ...
    public void removeChunk(int n, int n2) {
        int n3 = RegionLoader.getChunkOffset(n, n2);
        Integer[] integerArray = this.locationTable.get(0);
        integerArray[0] = 0;
        integerArray[1] = 0;
        this.locationTable.put(n3, integerArray);
    }
    ...
    private void a() throws IOException {
        ...
        randomAccessFile.seek(0L);
        for (n = 0; n < 1024; ++n) {
            integerArray = this.locationTable.get(n);
    ...
    private int b() throws IOException {
        ...
        for (var entry : this.locationTable.entrySet()) {
        ...
        n = 2;
        var object = treeMap.keySet().iterator();
        while (object.hasNext()) {
            int n4;
            n = n4 = ((Integer)object.next()).intValue();
            int n5 = treeMap.get(n4);
            if (n4 - n3 > 1) {
                n2 += n4 - n3 - 1;
            }
            if (n2 > 0) {
                randomAccessFile.seek(n4 << 12);
                var objectArray = new byte[4096];
