...
extends BlockFlowable {
    ...
    public void calculateState(boolean bl, boolean bl2, int n, Block block) {
        ...
        for (int k = 1; k < 42; ++k) {
            var vector3 = ((Vector3)location).getSide(blockFace, k);
            var object = this.level.getBlock(vector3);
        ...
        if (n2 > 0) {
            var vector3 = ((Vector3)location).getSide(blockFace, n2);
            var object = blockFace.getOpposite();
            blockTripWireHook.setFace((BlockFace) object);
            this.level.setBlock(vector3, blockTripWireHook, true, false);
            this.level.updateAroundRedstone(vector3, null);
            this.level.updateAroundRedstone(vector3.getSide(((BlockFace) object).getOpposite()), null);
        ...
        if (bl3 != bl5) {
            for (int k = 1; k < n2; ++k) {
                var object = ((Vector3)location).getSide(blockFace, k);
                block = blockArray[k];
