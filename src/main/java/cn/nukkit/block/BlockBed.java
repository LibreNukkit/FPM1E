...
implements Faceable {
        ...
        if (player != null) {
            ...
            if (!bl) {
                if ((player.getServer().bedSpawnpoints || player.getServer().suomiCraftPEMode()) && !player.getSpawn().equals(block)) {
                    var entityArray = new PlayerBedEnterEvent(player, this, true);
    ...
    public boolean onBreak(Item item) {
        ...
        if (block5 != null) {
            var eg = (block5.getDamage() & 8) == 8 ? block5.toItem() : null;
            if (this.getLevel().setBlock(block5, Block.get(0), true, true) && eg != null && this.canDropItem && this.getLevel().gameRules.getBoolean(GameRule.DO_TILE_DROPS)) {
                this.getLevel().dropItem(this.add(0.5, 0.5, 0.5), (Item)eg);
            }
        }
    ...
    private void a(Vector3 vector3, int n) {
        ...
        BlockEntity.createBlockEntity("Bed", this.getChunk(), compoundTag);
    }
