...
implements LoginChainData {
    ...
    private void a() {
        block10: {
            ...
            Map map = c.fromJson(new String(this.k.get(n), StandardCharsets.UTF_8), new MapTypeToken(null).getType());
            if (map.isEmpty() || !map.containsKey("chain") || ((List<?>)map.get("chain")).isEmpty()) {
                return;
            }
            List<String> list = (List<String>)map.get("chain");
            try {
    ...
    static {
        ...
        try {
            ...
            throw new AssertionError(generalSecurityException);
        }
    }
