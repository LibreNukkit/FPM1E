...
public class Binary {
    ...
    public static byte[] writeUUID(UUID uUID) {
        return Binary.appendBytes(Binary.writeLLong(uUID.getMostSignificantBits()), new byte[][]{Binary.writeLLong(uUID.getLeastSignificantBits())});
    }
        ...
        for (Map.Entry<Integer, EntityData> entry : map.entrySet()) {
            ...
            switch (entityData.getType()) {
                ...
                case 7: {
                    if (n < 560) {
                        if (n2 == 0) {
                            var object = ((LongEntityData)entityData).dataVersions;
                            ...
                            if (Server.getInstance().minimumProtocol != ProtocolInfo.CURRENT_PROTOCOL) {
                                Server.getInstance().getLogger().debug("Invalid LongEntityData dataVersions for DATA_FLAGS, reverting to non-multiversion compatible flags: expected 3, got " + (object == null ? 0 : (object).length));
                            }
                        } else if (n2 == 92) {
                            var  object = ((LongEntityData)entityData).dataVersions;
                            ...
                            if (Server.getInstance().minimumProtocol != ProtocolInfo.CURRENT_PROTOCOL) {
                                Server.getInstance().getLogger().debug("Invalid LongEntityData dataVersions for DATA_FLAGS_EXTENDED, reverting to non-multiversion compatible flags: expected 1, got " + (object == null ? 0 : (object).length));
                            }
                        }
                    }
                ...
                case 8: {
                    binaryStream.putLFloat(((Vector3fEntityData) entityData).x);
                    binaryStream.putLFloat(((Vector3fEntityData) entityData).y);
                    binaryStream.putLFloat(((Vector3fEntityData) entityData).z);
                }
            }
        }
