...
extends Thread {
            ...
            if (l2 != 0L && (l = System.currentTimeMillis() - l2) > this.time) {
                if (this.a.isRunning()) {
                    block12: {
                        ...
                        Watchdog.a("--------- Server stopped responding ---------", mainLogger, stringBuilder);
                        Watchdog.a("Last response " + l3 + " seconds ago", mainLogger, null);
                        Watchdog.a("---------------- Main thread ----------------", mainLogger, null);
                        Watchdog.a(ManagementFactory.getThreadMXBean().getThreadInfo(this.a.getPrimaryThread().getId(), Integer.MAX_VALUE), mainLogger, stringBuilder);
                        Watchdog.a("---------------- All threads ----------------", mainLogger, stringBuilder);
                        ...
                        for (int k = 0; k < threadInfoArray.length; ++k) {
                            if (k != 0) {
                                Watchdog.a("------------------------------", mainLogger, stringBuilder);
                            }
                            Watchdog.a(threadInfoArray[k], mainLogger, stringBuilder);
                        }
                        Watchdog.a("---------------------------------------------", mainLogger, null);
                        try {
    ...
    private static void a(ThreadInfo threadInfo, Logger logger, StringBuilder stringBuilder) {
        ...
        Watchdog.a("\tPID: " + threadInfo.getThreadId() + " | Suspended: " + threadInfo.isSuspended() + " | Native: " + threadInfo.isInNative() + " | State: " + threadInfo.getThreadState(), logger, stringBuilder);
