...
extends VanillaCommand {
    ...
    public boolean execute(CommandSender commandSender, String string, String[] stringArray) {
        ...
        if (Objects.equals(player, commandSender)) {
            commandSender.sendMessage(new TranslationContainer(TextFormat.RED + "%commands.message.sameTarget"));
            return true;
        }
        ...
        if (stringBuilder.length() > 512) {
            commandSender.sendMessage(TextFormat.RED + "The message is too long");
            return true;
        }
